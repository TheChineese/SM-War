...
// ConVar Handle
ConVar g_cBuy = null;
ConVar g_cBuyTime = null;
ConVar g_cLimitRounds = null;
ConVar g_cDurationRounds = null;
ConVar g_cAttackTeam = null;
ConVar g_cLimitTime = null;
ConVar g_cRulesTime = null;
ConVar g_cShowRules = null;
ConVar g_cCooldownTime = null;
ConVar g_cAntiStuckTime = null;
ConVar g_cDFFreezeTime = null;
ConVar g_cDFBuyTime = null;
ConVar g_cWarMoney = null;
...
// Timer Handle
Handle g_tCooldownTimer = null;
Handle g_tAntiStuckTimer = null;
Handle g_tBuyTimer = null;
Handle g_tLimitTimer = null;
...
{
	if (g_wsCurrentWarStatus == WS_PROCESS) // Is War in progress?
	{
		if (g_iDurationRounds <= g_iWarRounds) // Played enough War for the moment?
		{
			if (g_bDebug)
			{
		...
		{
			g_iWarRounds = g_iWarRounds + 1; // We play another War roun :)
		}
	}
	else
	{
		if (g_iMinNoWarRounds >= 1) // Is War still limited?
		{
			g_iMinNoWarRounds = g_iMinNoWarRounds - 1; // Lower the limit counter
		}
		else if (g_iMinNoWarRounds <= 0 && g_wsCurrentWarStatus == WS_INITIALISING) // No Limit and waiting to play?
		{
			SetStatus(WS_PROCESS);  // Lets play next round ;)
		}
	}
...
public Action Event_RoundEnd_Callback(Handle event, char[] name, bool dontBroadcast)
{
	...
	SaveKillTimer(g_tAntiStuckTimer);
	SaveKillTimer(g_tCooldownTimer);
	SaveKillTimer(g_tLimitTimer);
}
...
public Action Timer_Cooldown(Handle p_hTimer)
{
	...
	g_bIsCooldown = false;
	CheckGodMode(true);
	g_tLimitTimer = CreateTimer(GetConVarFloat(g_cLimitTime), Timer_Limit);
	
	// Call forward
	Call_StartForward(g_hWarCooldownForward);
	Call_Finish();
	
	g_tCooldownTimer = null;
}
...
void UpdateSetting(Handle convar = null) {
	...
	{
		g_bBuy = g_cBuy.BoolValue;
	}
	else if (convar == g_cShowRules)
	{
		g_bShowRules = g_cShowRules.BoolValue;
		if (g_bShowRules)
		{
			PrepareRules();
		}
	...
	{
		g_iLimitRounds = g_cLimitRounds.IntValue;
	}
	else if (convar == g_cDurationRounds)
	{
		g_iDurationRounds = g_cDurationRounds.IntValue;
	}
	else if (convar == g_cAttackTeam)
	{
		g_iAttackTeam = g_cAttackTeam.IntValue;
	}
	else if (convar == g_cCooldownTime)
	{
		g_fCooldownTime = g_cCooldownTime.FloatValue;
	}
	else if (convar == g_cAntiStuckTime)
	{
		g_fAntiStuckTime = g_cAntiStuckTime.FloatValue;
	}
	else if (convar == g_cDFBuyTime)
	{
		if (g_bIsWar)
		{
			// Holy shit this is useless :O
...
void PrepareRules()
{
	...
	if (FileExists(p_sPathConfig))
	{
		...
		g_pRules = CreatePanel();
		Format(p_sBuffer, sizeof(p_sBuffer), "%T", "RulesTitle", LANG_SERVER);
		g_pRules.SetTitle(p_sBuffer);
		
		File p_fFile = null;
		char p_sLine[256];
		p_fFile = OpenFile(p_sPathConfig, "r");
		if (g_bDebug)
		{
		...
		if (p_fFile != null)
		{
			while (!p_fFile.EndOfFile() && p_fFile.ReadLine(p_sLine, sizeof(p_sLine)))
			{
				g_pRules.DrawText(p_sLine);
				if (g_bDebug)
				{
			...
			g_pRules.DrawItem(p_sBuffer);
		}
		else
		{
			// Disable rules
			g_cShowRules.IntValue = 0;
			if (g_bDebug)
			{
...
void ShowRules()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && (GetClientTeam(i) == 2 || GetClientTeam(i) == 3))
		{
			...
			SendPanelToClient(g_pRules, i, PanelCallback_Rules, GetConVarInt(g_cRulesTime));
		}
	}
}
...
void TeleportToWar()
{
	...
	// Start the Cooldown
	g_tCooldownTimer = CreateTimer(g_fCooldownTime + g_cDFFreezeTime.FloatValue, Timer_Cooldown);
	g_bIsCooldown = true;
	CheckGodMode();
}
...
void AntiStuckTeam(int p_iTeam, bool p_bAddFreezetime = false)
{
	...
	if (p_bAddFreezetime)
	{
		g_tAntiStuckTimer = CreateTimer(g_fAntiStuckTime + GetConVarFloat(g_cDFFreezeTime), Timer_DisableAntiStuck, p_iTeam);
	}
	else
	{
		g_tAntiStuckTimer = CreateTimer(g_fAntiStuckTime, Timer_DisableAntiStuck, p_iTeam);
	}
}
...
// Credits for this go to Alliedmodders ( who made this btw? ) - Update to 1.7 API by me
